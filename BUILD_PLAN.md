...
- [x] **6.8** **Test tổng Giai đoạn 6:** đọc kỹ output AI của toàn bộ 6 kịch bản demo — không chỉ đúng JSON, mà giọng văn/logic tiếng Việt có đúng như 1 giám sát điều phối thật viết không (mục 19.0). Đây là bước ĐỌC BẰNG MẮT, không tự động hóa được — người dùng (bạn) nên tham gia đọc thử ở bước này.
  ĐÃ ĐỌC ĐỦ CẢ 6/6 KỊCH BẢN qua Gemini THẬT (không mock): Kịch bản 1 + bonus (bước 6.4/6.5), và 4 kịch bản còn lại chạy qua `scripts/test_option_generator_remaining.py` (thêm xe B03/B04 vào DB theo đúng mục 15 để test đủ). Kết quả:
  - Severity tính ra ĐÚNG kỳ vọng ở cả 4: KB2 `critical` (time_to_deadline=25<30), KB3 `warning` (không có priority_order, giao lại lần đầu), KB4 `serious` (has_priority_order=True qua `hop_dong_phat`), KB5 `serious` (time_to_deadline=70 rơi đúng khoảng 30-90).
  - Giọng văn tiếng Việt tự nhiên, đúng chất giám sát điều phối nói chuyện với đồng nghiệp (không phải dịch máy) ở toàn bộ 4 kịch bản.
  - Logic bám sát đúng "Consider when generating options" của từng sub_type trong mục 19.1: KB2 (road_closed) đưa ra phương án đổi tuyến + xe thay thế đúng như gợi ý; KB3 (customer_absent) đúng thứ tự cân nhắc gọi khách → chờ/tiếp tục → trả hàng/hẹn lại; KB4 (cancel_order) KHÔNG đề xuất liên hệ khách để đảo ngược hủy đơn (đúng quy tắc cấm trong prompt); KB5 (major_breakdown) tái hiện chính xác phương án "partial pickup ưu tiên đơn SLA gấp hơn (DH-501)" y hệt gợi ý trong prompt, không đề xuất việc kéo xe như 1 option (đúng "Towing... is informational, not a decision option").
  - Không có tuyên bố "đã thực hiện xong" ở bất kỳ option nào (đúng ROLE AND SCOPE system prompt) trên cả 6 kịch bản.
  - Mỗi kịch bản đều nhận 2-3 option phân biệt rõ ràng, đủ 6 field JSON schema, `sla_risk_remaining` giảm dần hợp lý theo mức độ "cứu" tình huống của từng phương án.
  Kết luận: chất lượng output AI đạt yêu cầu demo trên cả 6 kịch bản. Giai đoạn 6 hoàn tất.
...
- [x] **7.1** `ranker.py` — `calculate_score()` theo mục 7.
  Kiểm tra: chạy trên các options đã sinh ở Giai đoạn 6, điểm số tính đúng công thức, thay đổi `ranking_weights` → thứ hạng đổi theo đúng logic. ĐÃ KIỂM TRA — `core/ranker.py`. Diễn giải `normalize()` (mục 7 không định nghĩa chi tiết): min-max normalize TRONG PHẠM VI các option đang so sánh cùng nhau (1 exception/group), không phải toàn hệ thống — hợp lý vì ranking là bài toán so sánh tương đối cho 1 tình huống cụ thể. `scripts/test_ranker.py`: điểm tính tay khớp cả 3 option; đổi trọng số cost=0.9 → option rẻ nhất thắng; đổi trọng số time=0.9 → option nhanh nhất thắng; edge case 1 option (fallback thủ công) không chia-cho-0; danh sách rỗng không lỗi.
- [~] **7.2** `geocoder.py` — wrapper Google Geocoding + Distance Matrix (điền `GOOGLE_MAPS_API_KEY` thật ở bước này).
  ...
  Đã làm: `core/geocoder.py` viết đủ `geocode()` + `distance_matrix()`, dùng `httpx` gọi thẳng REST endpoint (không thêm thư viện `googlemaps` ngoài mục 16), tích hợp cache + graceful degradation NGAY TỪ ĐẦU (không phải thêm sau). Đã xác nhận code gọi ĐÚNG định dạng request thật tới Google — dùng key rác gọi API thật, Google trả `200 OK` với `status: REQUEST_DENIED, error_message: "The provided API key is invalid"` (không phải lỗi format/network) → chứng minh phần code/tích hợp đúng, chỉ còn thiếu 1 key hợp lệ.
  CÒN THIẾU: user báo `GOOGLE_MAPS_API_KEY` đang lỗi nên CHƯA thể verify "ra đúng tọa độ hợp lý" bằng key thật hoạt động — đây là phần duy nhất chưa kiểm tra được, không do lỗi code. Phiên sau khi có key hoạt động: chạy `scripts/test_geocoder.py` (bỏ phần mock key giả) với địa chỉ demo thật (vd "18 Phạm Hùng, Nam Từ Liêm") rồi tick `[x]`.
- [x] **7.3** `geocode_cache` — cache theo hash địa chỉ, không gọi API lại địa chỉ đã có.
  Kiểm tra: gọi geocode 2 lần cùng địa chỉ, lần 2 không tốn API call (kiểm tra qua log/số lần gọi thực tế). ĐÃ KIỂM TRA — `scripts/test_geocoder.py` mock `httpx.get` (đếm số lần gọi) vì key thật đang lỗi: gọi `geocode()` 2 lần cùng địa chỉ → lần 1 gọi API (đếm=1) và ghi vào `geocode_cache`, lần 2 đọc từ cache, đếm vẫn =1 (không gọi lại), trả đúng toạ độ. Cơ chế cache độc lập với việc key thật có hoạt động hay không — verify được đầy đủ dù chưa có key.
- [x] **7.4** Graceful degradation khi Maps API lỗi (mục 14).
  Kiểm tra: giả lập lỗi (sai key tạm) → hệ thống vẫn xử lý ngoại lệ, chỉ thiếu thông tin khoảng cách, báo rõ cho user. ĐÃ KIỂM TRA — thật ra đây chính là tình trạng THẬT hiện tại của company (key rỗng/lỗi), không phải giả lập: `geocode()`/`distance_matrix()` với key rỗng → trả `None` sạch sẽ, không raise; với key sai gọi THẬT tới Google (không mock) → Google trả `REQUEST_DENIED`, code bắt đúng và trả `None`, không crash. `option_generator.build_context` đã có sẵn `distance_info: None` làm điểm nối graceful degradation (mục 14) từ Giai đoạn 6 — dispatcher/LLM chỉ thiếu thông tin khoảng cách, không chặn toàn bộ luồng xử lý ngoại lệ.
- [x] **7.5** Nối toàn bộ pipeline đúng thứ tự: `rule_engine → impact_analyzer → geocoder → option_generator → ranker` (mục 11) trong `job_processor.py`.
  ...
  ĐÃ KIỂM TRA — `geocoder.distance_matrix` nối vào `option_generator.build_context` (điền `distance_info`, tự graceful degradation về `None` do key Maps đang lỗi — đúng mục 14); `ranker.rank_options` nối vào `job_processor._process_job` sau khi persist option (rank cần TOÀN BỘ tập option để normalize, nên chạy sau persist chứ không chạy trong lúc persist từng cái). `scripts/test_stage7_e2e.py` chạy qua `job_processor.process_pending_jobs()` thật (không gọi tắt qua option_generator) cho đủ 6 kịch bản (5 đơn lẻ + 1 group) — severity đúng kỳ vọng 100%, mọi option đều có `rank`+`score`, score giảm dần đúng thứ tự rank, cả 2 exception thành viên nhóm bonus đều chuyển `awaiting_decision`. 29/29 PASS.
  PHÁT HIỆN + SỬA 1 bug thật: `_process_job` nhánh `analyze_group` chỉ set `status='awaiting_decision'` cho exception mà `job.exception_id` tình cờ trỏ tới, KHÔNG cập nhật các thành viên còn lại trong `exception_groups.exception_ids` — vi phạm đúng tinh thần "1 quyết định phối hợp" của combined mode (mục 5.3, 10): thành viên B (C02/major_breakdown) bị kẹt ở `analyzing` mãi dù nhóm đã có phương án. Sửa bằng cách lặp qua toàn bộ `group.exception_ids` để cập nhật status. Lỗi này CHỈ lộ ra khi test qua `job_processor` thật với ≥2 exception trong nhóm — các test Giai đoạn 6 trước đó gọi thẳng `option_generator.generate_options_for_group` nên không đi qua đoạn cập nhật status này.
...
- **Giai đoạn 6** (2026-09-02): Xong. `llm_adapter.py` dùng SDK `google-genai` (đổi khỏi `google-generativeai` đã bị Google deprecated hoàn toàn — xem ghi chú 6.1), seed 16 prompt (15 theo yêu cầu + `group`), `option_generator.py` (build_context + gọi LLM thật + retry 3 lần + parse JSON), `llm_usage.py` (log + hard limit 100 calls/ngày), fallback graceful khi LLM lỗi (job vẫn `done`, tạo option placeholder, thêm endpoint `POST /api/exceptions/{id}/manual-option`). Đọc bằng mắt output Gemini THẬT (không mock) trên ĐỦ CẢ 6/6 kịch bản demo mục 15 — severity đúng kỳ vọng 100%, giọng văn tiếng Việt tự nhiên đúng chất giám sát điều phối, logic bám sát prompt từng sub_type (chi tiết ở ghi chú bước 6.8). Phát hiện + sửa 1 bug thật trong `_infer_conflict_signals` (viết lại logic riêng thay vì tái dùng `conflict_detector.detect_conflict`, khiến bỏ sót tín hiệu).
- **Giai đoạn 7** (2026-09-02): Xong 7.1/7.3/7.4/7.5; 7.2 còn `[~]` (code đúng, Google trả `REQUEST_DENIED` với key rác — chứng minh format request đúng, chỉ chưa verify tọa độ vì `GOOGLE_MAPS_API_KEY` của user đang lỗi thật). `ranker.py` (`calculate_scores`/`rank_options`, min-max normalize trong phạm vi 1 batch option so sánh cùng nhau), `geocoder.py` (Geocoding + Distance Matrix qua httpx, cache + graceful degradation thiết kế từ đầu, không phải thêm sau). Nối full pipeline `rule_engine→impact_analyzer→geocoder→option_generator→ranker` vào `job_processor.py`, test end-to-end thật qua `process_pending_jobs()` cho đủ 6 kịch bản demo (29/29 PASS). Phát hiện + sửa 1 bug thật: nhánh `analyze_group` chỉ cập nhật status cho 1 exception (do `job.exception_id` trỏ tới) thay vì CẢ nhóm, vi phạm tinh thần "1 quyết định phối hợp" (mục 5.3, 10) — lỗi chỉ lộ ra khi test qua `job_processor` thật với ≥2 exception trong nhóm, không lộ ở Giai đoạn 6 vì lúc đó gọi tắt thẳng `option_generator`.
